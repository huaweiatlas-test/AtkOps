...
Status TFcustomExpInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomExpBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    ...
    std::string kernelName = "custom_exp" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomExpInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomExpBuildTeBin)                // Build Te op binary function
