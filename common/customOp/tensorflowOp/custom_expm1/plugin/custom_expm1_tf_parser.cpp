...
Status TFcustomExpm1InferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomExpm1BuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_expm1";
    std::string funcName = "custom_expm1";
    std::string kernelName = "custom_expm1" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomExpm1InferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomExpm1BuildTeBin)                // Build Te op binary function
