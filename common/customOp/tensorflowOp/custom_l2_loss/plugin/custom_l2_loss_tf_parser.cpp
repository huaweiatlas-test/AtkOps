...
Status TFcustomLossInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomLossBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_l2_loss";
    std::string funcName = "custom_l2_loss";
    std::string kernelName = "custom_l2_loss" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomLossInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomLossBuildTeBin)                // Build Te op binary function
