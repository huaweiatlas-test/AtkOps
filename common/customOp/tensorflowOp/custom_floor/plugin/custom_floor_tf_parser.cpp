...
Status TFcustomFloorInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomFloorBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_floor";
    std::string funcName = "custom_floor";
    std::string kernelName = "custom_floor" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomFloorInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomFloorBuildTeBin)                // Build Te op binary function
