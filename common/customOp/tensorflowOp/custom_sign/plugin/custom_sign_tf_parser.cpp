...
Status TFcustomSignInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomSignBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_sign";
    std::string funcName = "custom_sign";
    std::string kernelName = "custom_sign" + std::to_string(inputDesc.GetShape().GetDim(0)) +
    ...
    std::map<ge::DataType, std::string> operation_map = { { ge::DT_UNDEFINED, "undefined" },
        { ge::DT_FLOAT,          "float32" },
    ...
    te::BuildTeCustomOp(te_bin_info.ddk_version,
                        op.GetName(),
                        realFilePath,
                        funcName,
                        "(i,i,i,i),s, s,O",
...
REGISTER_CUSTOM_OP("custom_sign")                       // custom_sign is the type name of the operator in the OM model. It can be specified randomly and
...
.InferShapeAndTypeFn(TFcustomSignInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomSignBuildTeBin)                // Build Te op binary function
