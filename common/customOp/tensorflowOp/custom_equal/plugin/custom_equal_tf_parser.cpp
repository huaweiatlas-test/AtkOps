...
Status TFcustomEqualInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomEqualBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);
    TensorDesc inputDesc2 = op.GetInputDesc(1);

    std::string filePath = "../operator/custom_equal";
    std::string funcName = "custom_equal";
    std::string kernelName = "custom_equal" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomEqualInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomEqualBuildTeBin)                // Build Te op binary function
