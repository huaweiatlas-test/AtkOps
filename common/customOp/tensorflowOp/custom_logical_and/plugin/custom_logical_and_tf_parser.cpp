...
Status TFcustomLogicalAndInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
    ...
    if (ProduceShape(dims1, dims2, output_dims) == 0) { // Shape broadcast
        return FAILED;
    }

    for (size_t i = 0; i < output_dims.size(); i++) {
        shape1.SetDim(i, output_dims[i]);
    }
...
Status TFcustomLogicalAndBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);
    TensorDesc inputDesc2 = op.GetInputDesc(1);

    std::string filePath = "../operator/custom_logical_and";
    std::string funcName = "custom_logical_and";
    std::string kernelName = "custom_logical_and" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomLogicalAndInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomLogicalAndBuildTeBin)                // Build Te op binary function
