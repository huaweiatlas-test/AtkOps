...
Status TFcustomSubtractInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomSubtractBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);
    TensorDesc inputDesc2 = op.GetInputDesc(1);

    std::string filePath = "../operator/custom_subtract";
    std::string funcName = "custom_subtract";
    std::string kernelName = "custom_subtract" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomSubtractInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomSubtractBuildTeBin)                // Build Te op binary function
