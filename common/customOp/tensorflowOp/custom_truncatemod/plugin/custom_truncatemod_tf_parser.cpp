...
Status TFcustomTruncatemodInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomTruncatemodBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);
    TensorDesc inputDesc2 = op.GetInputDesc(1);

    std::string filePath = "../operator/custom_truncatemod";
    std::string funcName = "custom_truncatemod";
    std::string kernelName = "custom_truncatemod" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomTruncatemodInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomTruncatemodBuildTeBin)                // Build Te op binary function
