...
Status TFcustomSquaredDifferenceInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomSquaredDifferenceBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);
    TensorDesc inputDesc2 = op.GetInputDesc(1);

    std::string filePath = "../operator/custom_squared_difference";
    std::string funcName = "custom_squared_difference";
    std::string kernelName = "custom_squared_difference" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomSquaredDifferenceInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomSquaredDifferenceBuildTeBin)                // Build Te op binary function
