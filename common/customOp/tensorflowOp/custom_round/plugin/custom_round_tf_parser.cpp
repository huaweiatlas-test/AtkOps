...
Status TFcustomRoundInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomRoundBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_round";
    std::string funcName = "custom_round";
    std::string kernelName = "custom_round" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomRoundInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomRoundBuildTeBin)                // Build Te op binary function
