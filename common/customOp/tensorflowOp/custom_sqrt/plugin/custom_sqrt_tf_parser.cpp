...
Status TFcustomSqrtInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomSqrtBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_sqrt";
    std::string funcName = "custom_sqrt";
    std::string kernelName = "custom_sqrt" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomSqrtInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomSqrtBuildTeBin)                // Build Te op binary function
