...
def custom_logical_and(shape_x, shape_y, dtype,
                       kernel_name="cce_tf_logical_and", need_build=False,
                       need_print=False):
    """
    ...
    data1 = tvm.placeholder(shape_x, dtype=dtype.lower(), name="data1")
    data2 = tvm.placeholder(shape_y, dtype=dtype.lower(), name="data2")

    with tvm.target.cce():
        data1_tmp1 = te.lang.cce.broadcast(data1, shape_max)
        data1_tmp2 = te.lang.cce.broadcast(data2, shape_max)

        min_value = tvm.const(0, dtype=dtype.lower())
        res = tvm.compute(shape_max, lambda *i: tvm.select(
            tvm.all(tvm.any(data1_tmp1(*i) > min_value,
                            data1_tmp1(*i) < -min_value),
                    tvm.any(data1_tmp2(*i) > min_value,
