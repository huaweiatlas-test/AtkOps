...
Status TFcustomMaximumInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomMaximumBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);
    TensorDesc inputDesc2 = op.GetInputDesc(1);

    std::string filePath = "../operator/custom_maximum";
    std::string funcName = "custom_maximum";
    std::string kernelName = "custom_maximum" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomMaximumInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomMaximumBuildTeBin)                // Build Te op binary function
