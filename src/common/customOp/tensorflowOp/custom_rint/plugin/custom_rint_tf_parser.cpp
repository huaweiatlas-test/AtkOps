...
Status TFcustomRintInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomRintBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_rint";
    std::string funcName = "custom_rint";
    std::string kernelName = "custom_rint" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomRintInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomRintBuildTeBin)                // Build Te op binary function
