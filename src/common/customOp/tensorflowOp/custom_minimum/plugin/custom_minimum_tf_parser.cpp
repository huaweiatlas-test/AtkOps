...
Status TFcustomMinimumInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomMinimumBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);
    TensorDesc inputDesc2 = op.GetInputDesc(1);

    std::string filePath = "../operator/custom_minimum";
    std::string funcName = "custom_minimum";
    std::string kernelName = "custom_minimum" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomMinimumInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomMinimumBuildTeBin)                // Build Te op binary function
