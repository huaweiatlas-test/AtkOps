...
Status TFcustomSquareInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomSquareBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_square";
    std::string funcName = "custom_square";
    std::string kernelName = "custom_square" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomSquareInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomSquareBuildTeBin)                // Build Te op binary function
