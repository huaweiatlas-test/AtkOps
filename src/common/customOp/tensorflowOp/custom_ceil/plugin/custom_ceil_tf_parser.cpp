	...
	* Description: ceil_tf_parser.cpp
	* Create: 2019-10-29
    * Author:
	* Notes:
    */
...
Status TFcustomCeilInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomCeilBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_ceil";
    std::string funcName = "custom_ceil";
    std::string kernelName = "custom_ceil" + std::to_string(inputDesc.GetShape().GetDim(0)) 
	                                       + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
// custom_ceil is the type name of the operator in the OM model. It can be specified randomly and 
// cannot be the same as an existing type name. It is case sensitive.
REGISTER_CUSTOM_OP("custom_ceil")                     
...
.InferShapeAndTypeFn(TFcustomCeilInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomCeilBuildTeBin)                // Build Te op binary function
.ImplyType(ImplyType::TVM)                           // Implementation type. Enumerated type, 
                                                     // The options are as follows: TVM, AI_CPU.
