...
Status TFcustomLogInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomLogBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    ...
    std::string kernelName = "custom_log" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomLogInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomLogBuildTeBin)                // Build Te op binary function
