...
def custom_sqrt(shape, dtype, kernel_name="cce_sqrt", need_build=False,
                need_print=False):
    """
    calculate data ** 0.5, calulation method exp(0.5*log(data)), calculating
    ...
    if shape_len > max_dim:
        raise RuntimeError(
            "sqrt_cce only support up to %d dimensions while the shape's\
    ...
    output = tvm.extern(shape, [data_input, p_shape], lambda ins, outs:
                        tvm.call_extern("int32_t", device_api, block_num, block_idx,
                                        v_ndim, ins[1].access_ptr(
                                            "r"),  # shape
