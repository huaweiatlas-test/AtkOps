...
Status TFcustomLogicalNotInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &v_output_desc)
{
...
Status TFcustomLogicalNotBuildTeBin(const ge::Operator &op, TEBinInfo &te_bin_info)
{
    // ### Parse input tensor description
    TensorDesc inputDesc = op.GetInputDesc(0);

    std::string filePath = "../operator/custom_logical_not";
    std::string funcName = "custom_logical_not";
    std::string kernelName = "custom_logical_not" + std::to_string(inputDesc.GetShape().GetDim(0)) + std::to_string(inputDesc.GetShape().GetDim(DOMI_COMMON_ONE));
...
.InferShapeAndTypeFn(TFcustomLogicalNotInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(TFcustomLogicalNotBuildTeBin)                // Build Te op binary function
