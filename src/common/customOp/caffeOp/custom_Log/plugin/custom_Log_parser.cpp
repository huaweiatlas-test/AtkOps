...
Status CaffecustomLogParseParams(const Message *opOrigin, ge::Operator &opDest)
{
...
Status CaffecustomLogInferShapeAndType(const ge::Operator &op, vector<ge::TensorDesc> &vOutputDesc)
{
...
Status CaffecustomLogBuildTeBin(const ge::Operator &op, TEBinInfo &teBinInfo)
{
    ...
    std::string filePath = "../operator/custom_Log";
    std::string funcName = "custom_Log";
    std::string kernelName = "custom_Log_" + std::to_string(input1Desc.GetShape().GetDim(0)) 
...
.ParseParamsFn(CaffecustomLogParseParams)              // Op parameters parse function
.InferShapeAndTypeFn(CaffecustomLogInferShapeAndType)  // Set output description and datatype function
.TEBinBuildFn(CaffecustomLogBuildTeBin)                // Build Te op binary function
