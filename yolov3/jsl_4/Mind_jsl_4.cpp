...
const static std::string SIMULATOR_FUNCTION = "Simulator_function";
const static std::string SIMULATOR_PERFORMANCE = "Simulator_performance";
...
*/
bool Mind_jsl_4::CheckNumber(const std::string& value)
{
...
*/
void Mind_jsl_4::GetSelectImages(const std::string& value)
{
    // if value is "", no select images
    if (value == "") {
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_jsl_4] select images info is null!");
        return;
    }
    std::string token;
    ...
    while (std::getline(tokenStream, token, ',')) {
        if (CheckNumber(token)) {
            ...
            if (frameId >= 0 && frameId < dataset_info_.size()) {
                ...
                HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_jsl_4] select image frame id must be in [0, %u), but the input is %u !", dataset_info_.size(), frameId);
            }
        } else {
            HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_jsl_4] select image info must be positive integer, but the input is '%s' !", token.c_str());
        }
    }
    if (select_images_.size() > 0) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] select %u images success!", select_images_.size());
    }
}
...
*/
void Mind_jsl_4::GetRandomImages()
{
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] select random %u images success!", data_config_->randomNumber);
}
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_jsl_4::MakeDatasetInfo()
{
    // get the data info file path
    std::string datainfoPath = data_config_->path;
    while (datainfoPath.back() == '/' || datainfoPath.back() == '\\') {
        datainfoPath.pop_back();
    }
    std::size_t datasetNameIndex = datainfoPath.find_last_of("/\\");
    std::string dataInfoPath = data_config_->path + "." + datainfoPath.substr(datasetNameIndex + 1) + "_data.info";
    ...
    while (count < totalFileNum) {
        ...
        std::getline(fin, lineContent);
        std::stringstream lineStrFormat(lineContent);
...
*/
void Mind_jsl_4::GetSelectOrRandomImages(const std::string& selectImages, const std::string& randomNumber)
{
    // clear vector
    ...
    if (data_config_->runMode == "random") {
        if (CheckNumber(randomNumber)) {
            ...
            if (randomNum > 0 && randomNum < dataset_info_.size()) {
                ...
                HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_jsl_4] random number must be in (0, %u), but the input is %u !", dataset_info_.size(), randomNum);
            }
        } else {
            HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_jsl_4] random number must be positive integer, but the input is '%s' !", randomNumber.c_str());
        }
    ...
    if (select_images_.size() == 0) {
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_jsl_4] There is no random or select images, run all images!");
...
*/
void Mind_jsl_4::GetBatchSize(const std::string& value)
{
    bool check = false;
    ...
    if (!check) {
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_jsl_4] batch size must be positive integer, but the input is '%s'. Use default batch size %u !", value.c_str(), data_config_->batchSize);
    }
}

HIAI_StatusT Mind_jsl_4::Init(const hiai::AIConfig& config, const  std::vector<hiai::AIModelDescription>& model_desc)
{
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] start init!");

    data_config_ = std::make_shared<DataConfig>();
    if (data_config_ == NULL) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] make shared for DataConfig error!");
        return HIAI_ERROR;
    }
    ...
    if (realpath(data_config_->path.c_str(), path) == NULL) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] can not find path!");
        return HIAI_ERROR;
    }
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] end init!");
    return ret;
}
...
*/
bool Mind_jsl_4::GetImageBuffer(const char* path, uint8_t* imageBufferPtr, uint32_t imageBufferLen, uint32_t frameId)
{
    bool ret = false;
    FILE* file = fopen64(path, "r");
    if (NULL == file) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] Error: open file %s failed", path);
        return ret;
    }
    do {
        unsigned long imageFseek = ((unsigned  long)frameId) * ((unsigned  long)imageBufferLen);
        if (0 != fseeko64(file, imageFseek, SEEK_SET)) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] fseeko64 offset = %u failed in GetImageBuffer", frameId * imageBufferLen);
            break;
        }
        if (imageBufferLen != fread(imageBufferPtr, 1, imageBufferLen, file)) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] fread length = %u failed in GetImageBuffer", imageBufferLen);
            break;
        }
        ret = true;
    } while (0);
...
*/
shared_ptr<EvbImageInfo> Mind_jsl_4::MakeEvbImageInfo(int index)
{
    ...
    if (evbImageInfo == NULL) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] make shared for EvbImageInfo error!");
        return NULL;
    }
    ...
    HIAI_StatusT getRet = hiai::HIAIMemory::HIAI_DMalloc(evbImageInfo->size, (void*&)imageBufferPtr);
    if (getRet != HIAI_OK || imageBufferPtr == NULL) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] DMalloc buffer error!");
        return NULL;
    }
    bool ret = GetImageBuffer(imageFullPath.c_str(), imageBufferPtr, dataset_info_[index].size, 0);
    if (!ret) {
        getRet = hiai::HIAIMemory::HIAI_DFree(imageBufferPtr);
        if (getRet != HIAI_OK) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] DFree buffer error!");
        }
        imageBufferPtr = NULL;
        return NULL;
    }
...
#if defined(IS_OI)
        HIAI_StatusT ret = hiai::HIAIMemory::HIAI_DVPP_DFree(ptr);
#else
        HIAI_StatusT ret = hiai::HIAIMemory::HIAI_DFree(ptr);
#endif
    if (ret != HIAI_OK) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] DFree buffer error!");
    }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_jsl_4::MakeImageInfo(NewImageParaT* imgData, int index)
{
    ...
    uint8_t* imageBufferPtr = NULL;
        ...
        HIAI_StatusT getRet = hiai::HIAIMemory::HIAI_DVPP_DMalloc(alignBufferSize, (void*&)imageBufferPtr);
#else
        imgData->img.size = dataset_info_[index].size;
        HIAI_StatusT getRet = hiai::HIAIMemory::HIAI_DMalloc(imgData->img.size, (void*&)imageBufferPtr);
#endif
    ...
    if (imageBufferPtr == NULL) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] alloc buffer error in MakeImageInfo");
        return HIAI_ERROR;
    }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_jsl_4::SendBatch(int batchId, int batchNum, std::shared_ptr<BatchImageParaWithScaleT> imageInfoBatch)
{
    ...
    do {
        ...
        if (hiaiRet == HIAI_QUEUE_FULL) {
            HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] queue full, sleep 200ms");
            usleep(SEND_DATA_INTERVAL_MS);
        }
    }while (hiaiRet == HIAI_QUEUE_FULL);

    if (hiaiRet != HIAI_OK) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] SendData batch %u failed! error code: %u", batchId, hiaiRet);
    }
    return hiaiRet;
}
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_jsl_4::SendEvbBatch(int batchId, int batchNum, std::vector<std::shared_ptr<EvbImageInfo>>& evbImageInfoBatch)
{
    HIAI_StatusT hiaiRet = HIAI_OK;
    for (unsigned int i = 0; i < evbImageInfoBatch.size(); ++i) {
        ...
        do {
            hiaiRet = SendData(DEFAULT_DATA_PORT, "EvbImageInfo", std::static_pointer_cast<void>(evbImageInfoBatch[i]));
            if (hiaiRet == HIAI_GRAPH_NO_USEFUL_MEMORY || hiaiRet == HIAI_GRAPH_INVALID_VALUE) {
                HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] hiai graph no useful memory, sleep 200ms");
                usleep(SEND_DATA_INTERVAL_MS);
            }
        }while (hiaiRet == HIAI_GRAPH_NO_USEFUL_MEMORY || hiaiRet == HIAI_GRAPH_INVALID_VALUE);

        if (hiaiRet != HIAI_OK) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] SendData image frame id %u failed! error code: %u", evbImageInfoBatch[i]->frame_ID, hiaiRet);
...
*/
int Mind_jsl_4::GetFrameId(int index)
{
    int frameId = index;
...
* free evb buffer
**/
void Mind_jsl_4::FreeEvbBuffer(const std::vector<std::shared_ptr<EvbImageInfo>>& evbImageInfoBatch, int start)
{
    ...
    for (unsigned int i = start; i < evbImageInfoBatch.size(); ++i) {
        if (evbImageInfoBatch[i]->pucImageData != NULL) {
            ...
            if (ret != HIAI_OK) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] free buffer error!");
            }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_jsl_4::RunOnSameSide()
{
    HIAI_StatusT ret = HIAI_OK;
    ...
    int i = 0;
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] run on %s, run %u images, batch size is %u", data_config_->target.c_str(), totalCount, data_config_->batchSize);
    for (int batchId = 0; batchId < batchNum && i < totalCount; batchId++) {
        ...
        if (imageInfoBatch == NULL) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] make shared for BatchImageParaWithScaleT error!");
            return HIAI_ERROR;
        }
        for (int j = 0 ; j < data_config_->batchSize && i < totalCount; j++) {
            ...
            if (ret != HIAI_OK) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_jsl_4] Error: make image info frame id %u for batch id %u failed! Stop to send images!", frameId, batchId);
                return ret;
            }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_jsl_4::RunOnDifferentSide()
{
    HIAI_StatusT ret = HIAI_OK;
    int totalCount = GetTotalCount();
    int batchNum = totalCount % data_config_->batchSize == 0 ? totalCount / data_config_->batchSize : totalCount / data_config_->batchSize + 1;
...
* @[return]: HIAI_StatusT
*/
bool Mind_jsl_4::isOnSameSide()
{
    if (data_config_->target == SIMULATOR_FUNCTION
        || data_config_->target == SIMULATOR_PERFORMANCE
        || data_config_->target == SIMULATOR_LOCAL
...
*/
HIAI_StatusT Mind_jsl_4::SendSentinelImage()
{
    HIAI_StatusT ret = HIAI_OK;
...
*/
HIAI_IMPL_ENGINE_PROCESS("Mind_jsl_4", Mind_jsl_4, INPUT_SIZE)
{
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] start process!");
    ...
    // send sentinel image
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] send sentinel image!");
    ret = SendSentinelImage();
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_jsl_4] end process!");
    return ret;
}
