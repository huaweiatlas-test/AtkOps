 ...
 */
bool GetAbsoluteFiles(const std::string &directory, std::vector<std::string> &filesAbsolutePath,
                      const std::vector<std::string> &fileFormat)
{
...
bool WriteOutputToFile(const OutputDataT &out, int index, const std::string &folder)
{
    ...
    std::string prefix = "result_files/" + folder + "/" + std::to_string(out.id) + "_" +
                         std::to_string(out.frame_id) + "_" + std::to_string(index);
